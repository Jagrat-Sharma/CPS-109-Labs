...
def get_next_int(it):
    ...
    the iterator, this function returns the next integer vlu
    ...
    try:
        while True:
            vlu = next(it)  # Get the next vlu from the iterator
            if isinstance(vlu, int):  # Check if the vlu is an integer
                return vlu
