...
def listexponential(n, base):
    ...
    sum = []
    for i in range(n): sum.append(base**i)
    return sum
...
def maxbytype(items):
    ...
    '''
    Assume that parameter 'items' is a_l heterogeneous list that
    may contain integers, floats, strings, and any other type.
    
    You should return a_l 3-tuple, where the first element is the
    ...
    return True if item is a_l float, False otherwise.
    
    FOOD FOR THOUGHT:
    Why might we use the special value 'None' when there is no
    instance of a_l particular type present in the list? Why not
